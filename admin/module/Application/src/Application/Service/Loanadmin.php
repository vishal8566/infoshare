...
//put your code here
    ...
    public function getloanData() {
        ...
        $select->join("tbl_candidate", 'tbl_candidate.candidate_id = tbl_deal.candidate_id', array('candidate_name'), $select::JOIN_LEFT);
        $select->join("tbl_user", 'tbl_user.user_id = tbl_deal.deal_createBy', array('user_office_name'), $select::JOIN_LEFT);
        $select->where(array('tbl_deal.deal_type' => '1'));
        $select->where->OR->equalTo('tbl_deal.deal_type', '3');
    ...
    public function getCountLoan($id, $search_form, $search_to) {
        ...
        $select->columns(array(new \Zend\Db\Sql\Expression('COUNT(DISTINCT(candidate_LLC_number)) as total_loan')));
        $statement = $this->sql->prepareStatementForSqlObject($select);
        $results = $statement->execute();
        $resultSet = new ResultSet();
        $resultSet->initialize($results);
    ...
    public function getCountDiscount($id, $search_form, $search_to) {
        $select = $this->sql->select("tbl_deal");
        $where = $select->where->nest;
        $where->greaterThanOrEqualTo("deal_due_date", $search_form);
        $where->lessThanOrEqualTo("deal_due_date", $search_to);
        $select->where(array('deal_createBy' => $id));
    ...
    public function getUpdate($val) {

        try {

            $select = $this->sql->select("tbl_deal");
            $select->where(array('deal_id' => $val));
            $statement = $this->sql->prepareStatementForSqlObject($select);
            $results = $statement->execute();
            $row = $results->current();
            ...
            if ($row['loan_status'] == '0') {
                ...
                $client['loan_status'] = '1';
            } else if ($row['loan_status'] == '1') {

                $client['loan_status'] = '2';
            } else {

                $client['loan_status'] = '1';
            }
            $update->set($client);
            $update->where(array('deal_id' => $val));
            $statement = $this->sql->prepareStatementForSqlObject($update);
            $result = $statement->execute();
            return true;
        } catch (\Exception $e) {
